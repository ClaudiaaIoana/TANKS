...
char proiectil1 = 'o', proiectil2 = 'o', proiectil = 'o';
...
replay:
	...
	while (joc_pierdut() != 1)
	{
		...
		//cat timp nu este apasata tasta space pt detonare jucatorul are optiunea de a se juca cu parametrii oricat
		while (stop != 1)
		{
			verifica_citire();
		}
		...
		verificare_viata();
		afisare_viata();
		//functia de verificare a vietii afiseaza si craterul format sub tanc deci este nevoia unei recalculari a pozitiei
		stergere_tanc1();
		deseneaza_tanc1(x_tanc1);
		stergere_tanc2();
		deseneaza_tanc2(x_tanc2);
...
void afiseaza_scena()
{
	...
	for_i
	{ scena[i][0] = (char)BARA_VERTICALA;
	scena[i][DIM_X] = (char)BARA_VERTICALA;
	}
		//laturile de sus si de jos
		for_j
	{
	scena[0][j] = (char)BARA_ORIZONTALA;
	scena[DIM_Y - 1][j] = (char)BARA_ORIZONTALA;
	}
		//colturile
	scena[0][0] = (char)COLT_STANGA_SUS;
	scena[0][DIM_X] = (char)COLT_DREAPTA_SUS;
	scena[DIM_Y - 1][0] = (char)COLT_STANGA_JOS;
	scena[DIM_Y - 1][DIM_X] = (char)COLT_DREAPTA_JOS;
	//afisaree scena
	for_i
	{ printf("\n");
		for_j
...
void nume_jucatori()
{
	...
	gotoXY(45, 16);
	printf("Name player1:");
	scanf("%s", &player1);//jucatorul 1 isi introduce numele si acesta va fi afisat in dreptul tancului sau
	gotoXY(45, 18);
	printf("Name player2:");
	scanf("%s", &player2);//jucatorul 2 isi introduce numele
}
...
void deseneaza_landscape()
{
	...
	for (int j = DIM_Y - 1; j > DIM_Y - 13; j--)
	{
		...
		for (int i = 1; i < DIM_X; i++)
		{
			...
			{
				printf("%c", HASUR);
				scena[j - 1][i] = (char)HASUR;//atribuite si in spatele, in matrice
			}
			else printf(" ");
		}
	}
...
void deseneaza_tanc1(int x)
{
	...
	for (int i = 0; i < 4; i++)
	{
		gotoXY(x + i, y);
		printf("%c", BAZA_TANC);
		scena[y - 1][x + i] = (char)BAZA_TANC;
	}
	for (int i = 1; i < 3; i++)
	{
		gotoXY(x + i, y - 1);
		printf("%c", CORP_TANC);
		scena[y - 2][x + i] = (char)CORP_TANC;
	}
	gotoXY(x + 3, y - 1);
	printf("/");
	scena[y - 2][x + 3] = '/';
	...
	{//dublura folosita in cadrul functiilor trigonometrice
		...
		{  //afisarea proiectilului cu efect de urcare cat timp nu s-a atins inaltimea maxima, proiectilul nu s-a ciocnit inca de teren si nu a iesit fin chenar 
			while ((drum != 0 || urcare != 0) && scena[y_proiectil_nou - 1][x_proiectil_nou] == ' ' && x_proiectil_nou < DIM_X - 2 && x_proiectil_nou>0)
			{
				...
				gotoXY(x_proiectil_nou, y_proiectil_nou);
				printf("%c", proiectil);
				scena[y_proiectil_nou - 1][x_proiectil_nou] = proiectil;
	...
	if (_kbhit())
	{
		...
		case '1'://afiseaza proiectilul 1 si schimba in spate parametrii pentru pregatirea lansarii, dar nu lanseaza
			if (tura % 2 == 1)
			{
				...
				nr_proiectil = nr_proiectil1[tip_proiectil1 - 1];
				proiectil = proiectil1 = 'o';
			}
			else
			{
				tip_proiectil = tip_proiectil2 = 1;
				nr_proiectil = nr_proiectil2[tip_proiectil2 - 1];
				proiectil = proiectil2 = 'o';
			}
		...
		case '3'://afiseaza proiectilul 3 si schimba in spate parametrii pentru pregatirea lansarii, dar nu lanseaza
			if (tura % 2 == 1)
			{
				...
				nr_proiectil = nr_proiectil1[tip_proiectil1 - 1];
				proiectil = proiectil1 = (char)PROIECTIL_AVION;
			}
			if (tura % 2 == 0)
			{
				tip_proiectil = tip_proiectil2 = 3;
				nr_proiectil = nr_proiectil2[tip_proiectil2 - 1];
				proiectil = proiectil2 = (char)PROIECTIL_AVION;
			}
			afisare_tip_proiectil();
			break;
...
{
	int i = 0;
	/*pentru proiectilele 1 si 2 exista cazul exceptional cand proiectilul va avea ca pozitie viitoare un punct din interiorul landsacep - ului
	 se studiaza acest caz dupa care cazul usual cu proiectilul pe suprafata terenului*/
	if (scena[y_proiectil_nou - 1][x_proiectil_nou] == (char)HASUR && scena[y_proiectil_nou - 2][x_proiectil_nou] == (char)HASUR && (tip_proiectil == 1 || tip_proiectil == 2))
	{
		if (u < 90)
		{
			while (scena[y_proiectil_nou - 1 - i][x_proiectil_nou - i] == (char)HASUR)//cat timp proiectilul nu este deasupra terenului coordonatele acestuia urca pe lateral
				i++;
			i--;//pentru a trece la primul caracter de la suprafata terenului, unde se va face afisarea craterului
			if (tip_proiectil == 1)
			{
				//tipul de proiectil 1 face un crater de dimenziunea unui singur caracter
			...
			{
				// tipul de proiectil 2 face un crater tip piramida rasturnata cu baza de 5 caractere si varful de un caracter
		...
		//pentru un unghi >90 se aplica acelasi procedeu dar inversat la aflarea coordonatelor de la suprafara terenului, traiectoria fiinf opusa
		if (u > 90)
		{
			while (scena[y_proiectil_nou - 1 - i][x_proiectil_nou + i] == (char)HASUR)//cat timp este in interioriul terenului coordonatele se muta pe diagonala spre stanga
				i++;
			i--;
			if (tip_proiectil == 1)
			{
				//afisarea craterului de un caracter
				gotoXY(x_proiectil_nou + i, y_proiectil_nou - i);
				printf(" ");
				scena[y_proiectil_nou - 1 - i][x_proiectil_nou + i] = ' ';
			}
			...
			{
				//afisarea craterului de tip piramida
				for (int j = 0; j < 3 && y_proiectil_nou - i + j < DIM_Y; j++)
					for (int k = 0; k <= 2 - j && x_proiectil_nou + i - k > 0 && x_proiectil_nou + i + k < DIM_X; k++)
					{
						gotoXY(x_proiectil_nou + i - k, y_proiectil_nou - i + j);
	...
	else if (scena[y_proiectil_nou - 1][x_proiectil_nou] == (char)HASUR)//cazul uzual cu pozitia finala aproiectilului pe suprafata terenului
	{
		if (tip_proiectil == 1)
		{
			gotoXY(x_proiectil_nou, y_proiectil_nou);
	...
	if (tip_proiectil == 3)//proiectilul de tip 3/avion creeaza crater de tip special
	{
		int y = DIM_Y - 1, x;
		x = x_proiectil_nou;
		//calculeaza coordonata pe y de unde va fi afisat craterul
		...
		while (scena[y - 1][x + 1] == (char)HASUR || scena[y - 1][x + 2] == (char)HASUR || scena[y - 1][x - 1] == (char)HASUR || scena[y - 1][x - 2] == (char)HASUR)
			y--;
		y++;
		//afisarea craterului tip piramida
		...
		//afisarea unei ramificatii 
		y = DIM_Y - 1;
		x = x - 10;
		while (scena[y - 1][x] == (char)HASUR) y--;
		y++;
		for (int k = 0; k < 2 && x - k > 0 && x - k < DIM_X; k++)//crater cu latimea de 2 caractere
		{
			gotoXY(x - k, y);
			printf(" ");
			scena[y - 1][x - k] = ' ';
		}
		//afisarea ramificatiei 2
		y = DIM_Y - 1;
		x = x + 20;
		while (scena[y - 1][x] == (char)HASUR) y--;
		y++;
...
void afisare_combustibil()
{
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(consoleHandle, 176);
	//actualizeaza valoarea combustibilului dupa fiecare deplasare
	if (tura % 2 == 1)
	{
		gotoXY(15, 4);
		printf("  ");
...
void putere0()
{
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(consoleHandle, 189);
	//reseteaza la finalul turei puterea tancului
	if (tura % 2 == 1)
	{
		putere = 1;
		gotoXY(1, 5);
...
void verificare_viata()
{
	...
	{
		if (u == 90 || (x_proiectil_nou < x_tanc1 + 3 && x_proiectil_nou >= x_tanc1 - 3))//daca jucatorul se loveste singur
		{
			viata1--;
			y_proiectil_nou = y_tanc1 + 1;
			crater();
		}
		else if (x_proiectil_nou <= x_tanc2 + 3 && x_proiectil_nou >= x_tanc2 - 3)//daca este atins tancul advers se scade viata acestuia
		{
			viata2--;
	...
	if (tura % 2 == 1 && tip_proiectil == 2)
	{
		...
		else if (x_proiectil_nou <= x_tanc2 + 3 && x_proiectil_nou >= x_tanc2 - 3)//daca loveste tancul advers
			viata2 -= 4;
		else if (x_proiectil_nou <= x_tanc2 + 5 && x_proiectil_nou >= x_tanc2 - 5)//forta proiectilului 2 este mare si scade puncte de viata si cand este aproape fr tinta
		{
			viata2 -= 2;
			y_proiectil_nou = y_tanc2 + 1;
			crater();
	...
	{
		if (u == 90 || (x_proiectil_nou <= x_tanc2 + 3 && x_proiectil_nou >= x_tanc2 - 3))//tancul se loveste singur
		{
			viata2--;
			y_proiectil_nou = y_tanc2 + 1;
			crater();
		}
		else if (x_proiectil_nou <= x_tanc1 + 3 && x_proiectil_nou >= x_tanc1 - 3)//proiectilul atinge adversarul
		{
			viata1--;
			y_proiectil_nou = y_tanc1 + 1;
			crater();
	...
	if (tura % 2 == 0 && tip_proiectil == 2)
	{
		...
		else if (x_proiectil_nou <= x_tanc1 + 3 && x_proiectil_nou >= x_tanc1 - 3)//tancul loveste adversarul
			viata1 -= 4;
		else if (x_proiectil_nou <= x_tanc1 + 5 && x_proiectil_nou >= x_tanc1 - 5)//impactul fortei proiectilului de tip 2
		{
			viata1 -= 2;
			y_proiectil_nou = y_tanc1 + 1;
			crater();
	...
	if (tip_proiectil == 3)//proiectilul 3 isi atinge mereu tinta deci viata va fi scazuta automat
	{
...
void afisare_viata()
{
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(consoleHandle, 176);
	//dupa fiecare verificare se actualizeaza viata pe ecran
	gotoXY(9, 2);
	printf("  ");
	gotoXY(9, 2);
	printf("%d", viata1);
...
void deplasare_dreapta()
{
	...
	{
		stergere_tanc1();
		deseneaza_tanc1(x_tanc1 + 1);//desenarea cu o pozitie mai la dreapta
		X = x_tanc1;
		Y = y_tanc1;
		//setarea coordonatei de pornire a proiectilului nou
		x_proiectil_nou = x_proiectil_vechi = x_tanc1 + 3;
		y_proiectil_nou = y_proiectil_vechi = y_tanc1 - 2;
	}
	else if (tura % 2 == 0)
	...
	combustibil--;//scade automat combustibilul

}

void deplasare_stanga()
{
	//desenarea cu o pozitie la stanga
	if (tura % 2 == 1)
	{
		stergere_tanc1();
		...
		X = x_tanc1;
		Y = y_tanc1;
		x_proiectil_nou = x_proiectil_vechi = x_tanc1 + 3;//setarea coordonatelor noi pt proiectil
		y_proiectil_nou = y_proiectil_vechi = y_tanc1 - 2;
	}
	else if (tura % 2 == 0)
	{
		...
		X = x_tanc2;
		Y = y_tanc2;
		x_proiectil_nou = x_proiectil_vechi = x_tanc2 - 3;//setarea coordonatei de pornire a proiectilului nou
		y_proiectil_nou = y_proiectil_vechi = y_tanc2 - 2;
	}
	combustibil--;//scaderea automata a combustibilului
}

void stergere_tanc1()
{
	//pentru a nu ramane caractere inutile sau in plus in matricea din spate si pe ecran este necesata stergerea repetata a tancului
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(consoleHandle, 187);
	for (int i = 0; i < 4; i++)//sterge linia de sus a tancului
	{
		gotoXY(x_tanc1 + i, y_tanc1);
		printf(" ");
		scena[y_tanc1 - 1][x_tanc1 + i] = ' ';
	}
	for (int i = 1; i < 3; i++)//sterge linia de jos a tancului
	{
...
{
	//la fiecare schimbare a tasteor va afisa tipul de proiectil ce va fi lansat la apasarea tastei space si cate proiectile mai are jucatorul la dispozitie
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(consoleHandle, 176);
	gotoXY(13, 5);
	printf("                      ");
...
void sfarsit_joc()
{
	//in final aceasta functie va afisa castigatorul si preciza posibilitatea reluarii jocului
	gotoXY(50, 15);
	printf("GAME OVER");
	if (viata1 == 0)
	{
...
void avion()
{
	//aceasta functie implementeaza miscarea tancului, pe linia a 9-a
	HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	if (tura % 2 == 1)
		SetConsoleTextAttribute(consoleHandle, 180);
	if (tura % 2 == 0)
		SetConsoleTextAttribute(consoleHandle, 176);
	x_avion_nou = 1;
	//introducerea avionului in scena
	...
	//deplasarea avionului prin scena
	while (x_avion_nou < DIM_X - 4)
	{
		//daca avionul ajunge deasupra tancului advers este lansat proiectilul
		if (tura % 2 == 1)
			if (x_avion_nou + 1 == x_tanc2 - 1)
			{
				proiectil_avion();
				crater();
				//reafisarea tancurilor in eventualitatea in care s-au format cratere sub acestea
				stergere_tanc1();
				deseneaza_tanc1(x_tanc1);
				stergere_tanc2();
				deseneaza_tanc2(x_tanc2);
...
{
	//proiectilul pleaca de pe linia de coordonata y=9 si de pe coloana de deasupra tancului adversarului cazand liber in jos
	x_proiectil_nou = x_proiectil_vechi = x_avion_nou + 1;
	y_proiectil_nou = y_proiectil_vechi = 10;
	while (scena[y_proiectil_nou - 1][x_proiectil_nou] == ' ')//oprire cand atinge tancul
	{
		gotoXY(x_proiectil_nou, y_proiectil_nou);
		printf("%c", proiectil);
		scena[y_proiectil_nou - 1][x_proiectil_nou] = proiectil;
	...
	//stergerea in final a ultimei pozitii pe care a ramas afisat proiectilul
	gotoXY(x_proiectil_vechi, y_proiectil_vechi);
	printf(" ");
	scena[y_proiectil_vechi - 1][x_proiectil_vechi] = ' ';
}